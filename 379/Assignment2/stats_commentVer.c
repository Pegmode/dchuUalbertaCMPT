...
void writeEndStatLine(char* name, int count , char* comment,FILE* f){//write a single line to file for the end stats
    char writeBuffer[MAX_LOG_LINESIZE]; 
    sprintf(writeBuffer, "  %-12s   %4i                         %s\n", name, count, comment);
...
void writeEndThreadStatLine(int thread, int count, FILE* f){//write a single line for the thread portion of the end statss
    char writeBuffer[MAX_LOG_LINESIZE]; 
    sprintf(writeBuffer, "  Thread%3i       %3i                         //Number of ‘T’s completed by %i\n", thread, count, thread);
    fwrite(writeBuffer, strlen(writeBuffer), 1, f);
}
...
//ONLY CALL WHEN ALL THREADS HAVE COMPLETED
    ...
    char* summaryString = "Summary:                                      // All work complete\n";
    fwrite(summaryString, strlen(summaryString), 1, f);
    writeEndStatLine("Work", g_endStats->work, "// Producer: # of ‘T’ commands", f);
    writeEndStatLine("Ask", g_endStats->ask, "// Consumer: # of asks for work", f);
    writeEndStatLine("Receive", g_endStats->receive, "// Consumer: # work assignments", f);
    writeEndStatLine("Complete", g_endStats->complete, "// Consumer: # completed tasks", f);
    writeEndStatLine("Sleep", g_endStats->sleep, "// Producer: # of ‘S’ commands", f);
    ...
    char writeBuffer[MAX_LOG_LINESIZE];
    sprintf(writeBuffer, "Transactions per second:%3.2f                //%i pieces of work in %0.3f secs\n", transPerSec, work, totalTime);
    fwrite(writeBuffer, strlen(writeBuffer), 1, f);
...
void writeLogEntry(double time, int id, int q, char* type, int n, char* comment){
    char writeBuffer[MAX_LOG_LINESIZE];
    if(strcmp(type, "Ask") == 0 || strcmp(type, "End") == 0 ){//Ask 
        sprintf(writeBuffer, "  %-4.3lf  ID= %-3i        %-15s       %s\n",time ,id, type, comment);
    }
    else if(strcmp(type, "Sleep") == 0){//Sleep
        sprintf(writeBuffer, "  %-4.3lf  ID= %-3i        %-15s %-3i   %s\n",time ,id, type, n, comment);
    }
    else{//others
        sprintf(writeBuffer, "  %-4.3lf  ID= %-3i Q= %-3i %-15s %-3i   %s\n",time ,id, q, type, n, comment);
    }
...
void logAsk(int id){
    ...
    sprintf(comment, "// Thread %i asks for work",id);
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, -1, "Ask", -1, comment);
...
void logReceive(int id, int q, int n){
    char comment[MAX_COMMENT_SIZE];
    sprintf(comment, "// Thread %i takes work, n=%i",id ,n);
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, q, "Receive", n, comment);
    incEndStat(&g_endStats->receive);
}

void logWork(int id, int q, int n){
    char comment[MAX_COMMENT_SIZE];
    sprintf(comment, "// Parent receives work with n=%i",n);
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, q, "Work", n, comment);
...
void logComplete(int id, int q, int n){
    char comment[MAX_COMMENT_SIZE];
    sprintf(comment, "// Thread %i completes task, n=%i",id ,n);
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, q, "Complete", n, comment);
    incEndStat(&g_endStats->complete);
}

void logEnd(int id){
    char comment[MAX_COMMENT_SIZE];
    sprintf(comment, "// End of input for producer");
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, -1, "End", -1, comment);
}

void logSleep(int id, int n){
    char comment[MAX_COMMENT_SIZE];
    sprintf(comment, "// Parent sleeps, n=%i", n);
    double time = elapsedTime();//temp clamped value
    writeLogEntry(time, id, -1, "Sleep", n, comment);
