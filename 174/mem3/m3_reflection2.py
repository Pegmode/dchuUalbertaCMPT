      ...
      for row in self.board:
          for tile in row:
             if (self.tiles[0] == None) or (self.tiles[1] == None):
                  ...
                  if valid_move:
                      tile.click_tracker_handler()                  
                      if self.tiles[0] == None and self.tiles[1] == None:
                          self.tiles[0] = tile
                          self.tiles[0].set_show_clicks(True)
                      elif self.tiles[0] != None and self.tiles[1] == None:
                          self.tiles[1] = tile 
                          self.tiles[0].set_show_clicks(False)
                          self.tiles[1].set_show_clicks(True)
      ...
      if self.tiles[0] != None and self.tiles[1] != None:
         ...
         if self.tiles[0] != self.tiles[1]:
            for tile in self.tiles:
               tile.hide()
               self.tiles[1].set_show_clicks(False)
         self.tiles[0] = None
         self.tiles[1] = None 
...
class Tile: 
    ...
    def draw(self):
        ...
        if self.exposed and self.show_clicks:
            Tile.window.set_font_size(30)           
            Tile.window.get_surface().blit(self.image, self.rectangle) 
            Tile.window.draw_string(str(self.hidden_click_tracker),self.rectangle.x,self.rectangle.y) 
        elif self.exposed:
            Tile.window.get_surface().blit(self.image, self.rectangle) 
        else:
            Tile.window.get_surface().blit(Tile.hidden_image, self.rectangle)
     
    def click_tracker_handler(self):
       self.hidden_click_tracker += 1
       
    def set_show_clicks(self,value):
       self.show_clicks = value
